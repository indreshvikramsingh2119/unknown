...
def stop_serial():
    ...
    if ser is not None and ser.is_open:
        ...
        print("Serial not open or already closed.")
...
def show_history_page():
    ...
    #     full_row =  row
    for row_num, full_row in enumerate(serial_data_history, start=1):
    ...
    def save_selected_rows_as_pdf():
        ...
        headers_to_print = ["type", "A", "B", "C", "D", "E.", "F"]
    ...
    back_btn = tk.Button(btn_frame, text="Back ", font=("Segoe UI", 12, "bold"), bg="#0288d1", fg="white",
                         command=show_print_page, cursor="hand2", bd=0, relief="flat")
...
def show_print_page():
    ...
    logo_img = Image.open("images/dmk3.png").resize((120, 40))
    logo_photo = ImageTk.PhotoImage(logo_img)
    logo_label = tk.Label(root, image=logo_photo, bd=0)
    logo_label.image = logo_photo
    logo_label.place(x=35, y=35)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    fields = [
        ("Company Name:", ""),
        ("Company Address:", ""),
        ("Customer ID:",latest_serial_values[0] ),
        ("Mobile No:", latest_serial_values[1]),
        
        ("A:", latest_serial_values[2]),
        ("B:", latest_serial_values[3]),
        ("C:", latest_serial_values[4]),
        ("D:", latest_serial_values[5]),
        ("E:", latest_serial_values[6]),
        ("F:", latest_serial_values[7]),
        ]
    ...
    for i, (label_text, default) in enumerate(fields):
        ...
        if label_text == "Mobile No:":
            e = tk.Entry(left_frame, font=("Segoe UI", 13), width=15,
...
# --- BOX 1: 5-digit Section in White Frame ---
    ...
    # Internal entries
    tk.Entry(box2, font=("Segoe UI", 13), width=10).pack(pady=10,)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    auto_fields = [
        ...
        ("Total CV:",latest_serial_values [8])
    ]
    unit_labels = {
        "NA CV:": "cal",
        "SA CV:": "cal",
    ...
    for i, (label_text, default) in enumerate(auto_fields, start=2):
        ...
        e = tk.Entry(center_frame, font=("Segoe UI", 13), width=15)
        e.insert(0, default)
    ...
    # start_button = tk.Button(main_frame, text="Start", command=start_serial)
    # stop_button = tk.Button(main_frame, text="Stop", command=stop_serial)
    # history_button = tk.Button(main_frame, text="History", command=show_history_page)
    # print_button = tk.Button(main_frame, text="Print", command=show_print_page)
    ...
    # Start Button
    start_button = tk.Button(button_row, text="Start", font=("Segoe UI", 11, "bold"),
                            bg="#4caf50", fg="white", command=start_serial)
    start_button.grid(row=0, column=1, padx=10, ipadx=10, ipady=5)

    # Stop Button
    stop_button = tk.Button(button_row, text="Stop", font=("Segoe UI", 11, "bold"),
                            bg="#f44336", fg="white", command=stop_serial)
    stop_button.grid(row=0, column=2, padx=10, ipadx=10, ipady=5)

    # History Button
    history_button = tk.Button(button_row, text="History", font=("Segoe UI", 11, "bold"),
                            bg="#3f51b5", fg="white", command=show_history_page)
    history_button.grid(row=0, column=3, padx=10, ipadx=10, ipady=5)

    # # Optional: Print Button
    # print_button = tk.Button(button_row, text="Print", font=("Segoe UI", 11, "bold"),
    # #                         bg="#607d8b", fg="white", command=show_print_page)
    # print_button.grid(row=0, column=4, padx=10, ipadx=10, ipady=5)
    
    
    
    style = ttk.Style()
    style.configure("Custom.TMenubutton", font=("Segoe UI", 10, "bold"), background="#1a759f",
                    foreground="white", padding=7)
    style.map("Custom.TMenubutton",
            background=[("active", "#166086")],
            foreground=[("active", "white")])

    # --- Dropdown Button ---
    generate_dropdown = ttk.Menubutton(button_row, text="Report", style="Custom.TMenubutton")
    # generate_dropdown.grid(row=0, column=1, padx=8, ipadx=6, ipady=2)
    generate_dropdown.grid(row=0, column=5, padx=10, ipadx=10, ipady=5)
    
    # --- COM Port Dropdown ---
    # --- COM Port Dropdown ---
    com_label = tk.Label(button_row, text="COM Port:", bg="#f8f9fa", fg="#003333", font=("Segoe UI", 12, "bold"))
    com_label.grid(row=0, column=6, padx=(20, 5), pady=5, sticky="e")

    selected_port = tk.StringVar()
    com_port_menu = ttk.Combobox(button_row, textvariable=selected_port, width=12, font=("Segoe UI", 11), state="readonly")
    com_port_menu['values'] = [port.device for port in serial.tools.list_ports.comports()]
    com_port_menu.grid(row=0, column=7, padx=5, pady=5)
    
    # --- Baudrate Dropdown ---
    baud_label = tk.Label(button_row, text="Baudrate:", bg="#f8f9fa", fg="#003333", font=("Segoe UI", 12, "bold"))
    baud_label.grid(row=0, column=8, padx=(20, 5), pady=5, sticky="e")
    
    selected_baud = tk.StringVar()
    baudrate_menu = ttk.Combobox(button_row, textvariable=selected_baud, width=12, font=("Segoe UI", 11), state="readonly")
    baudrate_menu['values'] = ["9600", "19200", "38400", "57600", "115200"]
    baudrate_menu.set("9600")  # Default value
    baudrate_menu.grid(row=0, column=9, padx=5, pady=5)
...
bg_image = Image.open("images/fullwall.png")
bg_image = bg_image.resize((window_width, window_height), Image.LANCZOS)  
bg_photo = ImageTk.PhotoImage(bg_image)

# Create canvas and place background image
canvas = tk.Canvas(root, width=300, height=300,highlightthickness=0, bd=0, bg="#064E46")
canvas.pack(fill="both", expand=True)
canvas.create_image(0, 0, image=bg_photo, anchor="nw")



# ---------------- Draw Rounded Login Box in Center ----------------
def draw_rounded_box(canvas, x, y, width, height, radius=30, color="#064E46"):
    ...
    # 4 arcs (corners)
    canvas.create_arc(x1, y1, x1 + 2 * radius, y1 + 2 * radius, start=90, extent=90, fill=color, outline="")
    canvas.create_arc(x2 - 2 * radius, y1, x2, y1 + 2 * radius, start=0, extent=90, fill=color, outline="")
    canvas.create_arc(x2 - 2 * radius, y2 - 2 * radius, x2, y2, start=270, extent=90, fill=color, outline="")
    canvas.create_arc(x1, y2 - 2 * radius, x1 + 2 * radius, y2, start=180, extent=90, fill=color, outline="")

# Get center of screen
center_x = window_width // 2
center_y = window_height // 2

# Draw rounded rectangle box behind the frame
draw_rounded_box(canvas, center_x, center_y, 300, 450, radius=30, color="#064E46")


## Place your content frame exactly over the rounded box
frame = ctk.CTkFrame(master=canvas, fg_color="#064E46", corner_radius=30)
canvas.create_window(window_width//2, window_height//2, window=frame, width=300, height=450)

inner_frame = tk.Frame(frame, bg="#064E46")
inner_frame.pack(expand=True)  # This will vertically center contents


username_label = tk.Label(inner_frame, text="Username", font=("Segoe UI", 13,"bold"), bg="#064E46", anchor="w", fg="white")
username_label.pack(fill="x", padx=40)

username_canvas = tk.Canvas(inner_frame, width=220, height=35, bg="#064E46", bd=0, highlightthickness=0)
username_canvas.pack(padx=40, pady=(5, 10))
username_canvas.create_oval(0, 0, 35, 35, fill="#0A6C5A", outline="#0A6C5A")  # Match
username_canvas.create_oval(185, 0, 220, 35, fill="#0A6C5A", outline="#0A6C5A")
username_canvas.create_rectangle(17, 0, 203, 35, fill="#0A6C5A", outline="#0A6C5A")
username_entry = tk.Entry(username_canvas, font=("Segoe UI", 13), bd=0, relief=tk.FLAT,
                        bg="#0A6C5A", highlightthickness=0, fg="white", insertbackground="white")
username_entry.place(x=10, y=5, width=200, height=25)



password_label = tk.Label(inner_frame, text="Password", font=("Segoe UI", 13,"bold") ,bg="#064E46", anchor="w", fg="white")
password_label.pack(fill="x", padx=40)

password_canvas = tk.Canvas(inner_frame, width=220, height=35, bg="#064E46", bd=0, highlightthickness=0)
password_canvas.pack(padx=40, pady=(5, 15))
password_canvas.create_oval(0, 0, 35, 35, fill="#0A6C5A", outline="#0A6C5A")
password_canvas.create_oval(185, 0, 220, 35, fill="#0A6C5A", outline="#0A6C5A")
password_canvas.create_rectangle(17, 0, 203, 35, fill="#0A6C5A", outline="#0A6C5A")
password_entry = tk.Entry(password_canvas, font=("Segoe UI", 13), show="*", bd=0, relief=tk.FLAT,
                          bg="#0A6C5A", highlightthickness=0, fg="white", insertbackground="white")
...
def on_login_hover(event):
    login_canvas.itemconfig(login_button_rect, fill="#eeeeee")
def on_login_leave(event):
    login_canvas.itemconfig(login_button_rect, fill="white")

def on_login_click(event):
    login()

login_canvas = tk.Canvas(inner_frame, width=180, height=40, bg="#064E46", bd=0, highlightthickness=0)
...
login_button_rect = create_rounded_rect(login_canvas, 0, 0, 180, 40, r=20, fill="white")
login_text = login_canvas.create_text(90, 20, text="Login", fill="black", font=("Segoe UI", 13, "bold",))
...
main_heading = tk.Label(inner_page, text="", font=("Segoe UI", 22, "bold"), bg="white", fg="#1a1a1a")
...
def read_serial_data():
    global latest_serial_values, serial_data_history, entries
    from datetime import datetime
    ...
    while running and ser.is_open:
        try:
            ...
            if "REPORT" in line:
                ...
                buffer += line + "\n"
                if "G :" in line:  # Assuming report ends at G line
                    report_started = False
                    # Now parse the complete report
                    lines = buffer.splitlines()
                    data_map = {}
                    for l in lines:
                        if ":" in l:
                            key, value = l.split(":", 1)
                            data_map[key.strip()] = value.strip().split()[0]  # Only digits part

                    # Fill Customer ID and Mobile
                    # if "CUSTOMER ID" in data_map and "CUSTOMER ID:" in entries:
                    #     entries["CUSTOMER ID:"].delete(0, tk.END)
                    #     entries["CUSTOMER ID:"].insert(0, data_map["CUSTOMER ID"])
                    
                    if "CUSTOMER ID" in data_map:
                        val = data_map["CUSTOMER ID"]
                        if val.isdigit():
                            entries["Customer ID:"].delete(0, tk.END)
                            entries["Customer ID:"].insert(0, val)

                    # Mobile No
                    if "MOBILE NO." in data_map:
                        val = data_map["MOBILE NO."]
                        if val.isdigit() and len(val) <= 10:
                            entries["Mobile No:"].delete(0, tk.END)
                            entries["Mobile No:"].insert(0, val)

                    
                    
                    
                    # Fill A–F in left frame
                    for key in ['A', 'B', 'C', 'D', 'E', 'F']:
                        label = f"{key}:"
                        if label in entries and key in data_map:
                            entries[label].delete(0, tk.END)
                            entries[label].insert(0, data_map[key])

                    # Fill only G (Total CV) in center frame
                    if "G" in data_map and "Total CV:" in entries:
                        entries["Total CV:"].delete(0, tk.END)
                        entries["Total CV:"].insert(0, data_map["G"])

                    # Empty all other center fields
                    for field in ["CV of thread:", "CV of unbrunt wire:", "NA CV:", "SA CV:", "Other connection:"]:
                        if field in entries:
                    ...
                    time_str = now.strftime("%I:%M %p")
                    row_data = [date_str, time_str] + list(data_map.values())
                    serial_data_history.append(row_data)
...
#         try:
#             line = ser.readline().decode(errors='ignore').strip()
#             print("Received line:", line)  # Debug: serial data ka actual format dekhne ke liye
...
#                     #         try:
#                     #             float(val)
#                     #             entries[f"{key}:"].delete(0, tk.END)
#                     #             entries[f"{key}:"].insert(0, val)
#                     #         except:
#                     #             pass
...
#                     #         float(val)
#                     #         entries["Total CV:"].delete(0, tk.END)
#                     #         entries["Total CV:"].insert(0, val)
#                     #     except:
#                     #         pass
                    
                    
#                     # Correct matching keys: serial data ke keys ke saath match karen
#                     if "CUSTOMER ID" in data_map:
#                         val = data_map["CUSTOMER ID"]
#                         if val.isdigit():
#                             entries["Customer ID:"].delete(0, tk.END)
